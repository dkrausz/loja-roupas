...
const simplifyProductsSchema = z.object({
  publicId: z.string().min(1),
  quantity: z.number().positive().min(1),
});

const returnProductSchema = productSchema.omit({ id: true });
const createProductBodySchema = productSchema.omit({
  id: true,
  publicId: true,
  orders: true,
  storeId: true,
...
export { productSchema, returnProductSchema, createProductBodySchema, updateProductSchema, simplifyProductsSchema };
