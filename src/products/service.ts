...
import { IProductService, TCreateProductBody, TProduct, TReturnProduct, TUpdateProductBody } from "./interfaces";
import { productSchema, returnProductSchema } from "./schemas";
...
export class ProductService implements IProductService {
  ...
  public getOneProductComplete = async (publicId: string): Promise<TProduct> => {
    const product = await prisma.product.findFirst({ where: { publicId } });
    if (!product) {
      throw new AppError(404, "Product not found");
    }
  ...
  public getProducts = async (
    ...
    nextPage: string,
    search?: string
  ): Promise<paginationResponse<TReturnProduct>> => {
    let products;
    let count;
    if (search) {
